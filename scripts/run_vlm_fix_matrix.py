...
# Prevent Transformers from importing TensorFlow backend (removes common TF/XLA CUDA stderr spam).
...
def _run_single(
    ...
    with _suppress_stderr_fd(enabled=quiet):
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VLM-Fix matrix (model x game) and write combined summaries.")
    ...
    parser.add_argument("--hf-config", type=str, default="vlm_fix")
    ...
    parser.add_argument("--models", nargs="+", default=OPEN_SOURCE_10_MODELS)
    parser.add_argument("--games", nargs="+", default=GAMES)
    parser.add_argument("--prompt-types", nargs="+", choices=["direct", "cot"], default=["direct"])
    ...
    parser.add_argument("--image-text-orders", nargs="+", choices=["image_first", "text_first"], default=None)
...
def main() -> int:
    ...
    df, dataset_dir = load_instances_frame(
    ...
    prompt_types = sorted(set(str(x).strip() for x in args.prompt_types if str(x).strip()))
    if not prompt_types:
        raise ValueError("prompt_types cannot be empty")

    run_tag = str(args.run_tag).strip()
    if not run_tag:
        run_tag = "prompt-" + "-".join(prompt_types)

    df = df[df["game"].isin(args.games)].copy()
    df = df[df["prompt_type"].isin(prompt_types)].copy()
    if args.render_variants:
        df = df[df["render_variant"].isin(args.render_variants)].copy()
    if args.prompt_variants:
        df = df[df["prompt_variant"].isin(args.prompt_variants)].copy()
    if args.rule_variants:
        df = df[df["rule_variant"].isin(args.rule_variants)].copy()
    if args.image_text_orders:
        df = df[df["image_text_order"].isin(args.image_text_orders)].copy()
    if args.question_targets:
        df = df[df["question_target"].isin(args.question_targets)].copy()
    if df.empty:
        raise ValueError("No rows left after filtering selected games and prompt_types.")

    print(
        "[filter] rows after filters:"
        f" total={len(df)} | per_game={df.groupby('game').size().to_dict()}"
    )
    ...
    for i, (model_name, game) in enumerate(combos, start=1):
        ...
        out_jsonl = runs_dir / game / f"{_safe_model_name(model_name)}__{run_tag}.jsonl"
        combo_bar.set_postfix_str(f"{model_name} | {game}")

        print(
            f"[combo {i}/{total}] model={model_name} game={game} "
            f"prompt_types={','.join(prompt_types)} n={len(game_df)}"
        )
    ...
    pred_path = results_dir / f"predictions_all__{run_tag}.parquet"
    pred_all.to_parquet(pred_path, index=False)

    summary = _summarize(pred_all)
    summary_path = results_dir / f"summary_full_combo__{run_tag}.csv"
    ...
    by_rule_path = results_dir / f"summary_by_game_rule__{run_tag}.csv"
    by_rule.to_csv(by_rule_path, index=False)

    runlog = pd.DataFrame(run_rows)
    runlog_path = results_dir / f"run_log__{run_tag}.csv"
